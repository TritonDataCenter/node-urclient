...
send_file(options, callback)
{
	...
	infl.once('amqp_error', function (err) {
		err = new VError(err, 'amqp connection interrupted');
		err.server_uuid = options.server_uuid;
		fin(err);
	});
...
recv_file(options, callback)
{
	...
	mod_assert.optionalString(options.dst_file, 'options.dst_file');
	if (options.dst_file) {
		path = options.dst_file;
	} else {
		mod_assert.string(options.dst_dir, 'options.dst_dir');
		path = mod_path.join(options.dst_dir, options.server_uuid);
	}
	...
	infl.once('http_put', function (req, res, next) {
		...
		/*
		 * Receive our nominated file from the remote host.
		 */
		var output = mod_fs.createWriteStream(path);
		output.on('error', function (err) {
			res.send(500);
			next(false);
			fin(new VError(err, 'could not write output file "%s"',
			    options.src_file));
		});
		req.pipe(output);
...
exec(options, callback)
{
	...
	infl.once('amqp_error', function (err) {
		infl.complete();
		callback(new VError(err, 'failure for host %s: connection ' +
		    'interrupted', options.server_uuid));
	});
