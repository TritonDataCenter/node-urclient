...
_start_timeout(amqp)
{
	...
	mod_assert.number(self.urc_connect_timeout);

	/*
	 * (Re-)establish connection timeout:
	 */
	clearTimeout(self.urc_timeout);
	self.urc_timeout = setTimeout(function () {
		var err = new Error('URConnection connection timeout');
		amqp.emit('error', err);
...
_connect()
{
	...
	var inact_timeout = null;
	var reset_inact_timer = function () {
		clearTimeout(inact_timeout);
		inact_timeout = setTimeout(function () {
			self.urc_amqp.emit('error', new Error('no heartbeat ' +
			    'or data in 2.5 heartbeat intervals'));
	...
	self.urc_amqp.on('error', function (err) {
		if (give_up())
			return;
